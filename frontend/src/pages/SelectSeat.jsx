...
const SelectSeat = () => {
  ...
  const getSchedules = async () => {
    try {
      ...
      if (response.data.ok && response.data.data.movieSchedulesforDate.length > 0) {
        setScreen(response.data.data);
        setSelectedTime(response.data.data.movieSchedulesforDate[0]); // Chọn suất chiếu đầu tiên
      } else {
        console.error("Không có lịch chiếu cho ngày này");
      }
    } catch (err) {
      console.error(err);
    }
